...
class NWPU(BaseDataset):
    def __init__(self,
        ...
        self.class_weights = torch.FloatTensor([1]).to(device)
        ...
        # Chỉ đọc img_list nếu list_path tồn tại
    ...
    def __getitem__(self, index):
        ...
        points = np.array([]).astype('float32').reshape(-1, 2)
        ratio = 1.0
        ...
        if self.list_path is None or 'test' in self.list_path:
            image, pad_info = self.check_img(image, 32)
            image = self.input_transform(image)
            label = self.label_transform(points, image.shape[:2])
            image = image.transpose((2, 0, 1))
            return image.copy(), label, ratio, name, original_image, pad_info
        ...
        return image.copy(), label, np.array(size), [name, idx, resize_factor], original_image, None


    def check_img(self, img, div=32):
        h, w = img.shape[:2]
    ...
    def AI_tcrop_then_scale(self, image, points, name):
        ...
        idx = np.pad(idx, ((0, topk - len(idx))), constant_values=-1)
    ...
    def multi_scale_inference(self, model, image, scales=[1], flip=False):
        ...
        final_pred = torch.zeros([1, self.num_classes, ori_height, ori_width]).to(device)
        ...
        for scale in scales:
            ...
            if max(height, width) <= np.min(self.crop_size):
                ...
                preds = torch.zeros([1, self.num_classes, new_h, new_w]).to(device)
    ...
    def patch_forward(self, model, img, gt_map, num_patches):
